...
contract("TokenSale Contract", (accounts) => {
  ...
  it("Owner should be able to add User4,5,6 to the Sale Vesting Schedule", async () => {
    ...
    const userVestingData_4 = await tokenInstance.walletToVestAllocations(
      accounts[4],
      7
    );
    const userVestingData_5 = await tokenInstance.walletToVestAllocations(
      accounts[5],
      8
    );

    const userVestingData_6 = await tokenInstance.walletToVestAllocations(
      accounts[6],
      9
    );
    ...
    // User 1 checks
    assert.equal(
      userVestingData_4[0].toString(),
      "7",
      "Vesting Category Is wrongly assigned"
    );
    assert.equal(
      userVestingData_4[1].toString(),
      user4_amount.toString(),
      "Total Amount is wrongly assigned"
    );
    assert.equal(
      userVestingData_4[6].toString(),
      "0",
      "Total Amount claimed Is wrongly assigned"
    );
    assert.equal(userVestingData_4[7], true, "IsVesting Is wrongly assigned");
    assert.equal(
      userVestingData_4[8],
      false,
      "isTgeTokens Claimed is Wrong Is wrongly assigned"
    );

    // User 2 checks
    assert.equal(
      userVestingData_5[0].toString(),
      "8",
      "Vesting Category Is wrongly assigned for User 2"
    );
    assert.equal(
      userVestingData_5[1].toString(),
      user5_amount.toString(),
      "Total Amount is wrongly assigned for User 2"
    );
    assert.equal(
      userVestingData_5[6].toString(),
      "0",
      "Total Amount claimed Is wrongly assigned"
    );
    ...
    assert.equal(
      userVestingData_5[8],
      false,
      "IsVesting Is wrongly assigned for User 2"
    );
  });
  it("Transfer tokens to contract", async () => {
    ...
    const contractBalance = await tokenInstance.balanceOf(
      tokenInstance.address
    );
    assert.equal(
      contractBalance.toString(),
      intialSupply.toString(),
  ...
  it("User 6 should not be able to claim amount after 35 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user6_claims.toString(),
      "Claims do not match for 35 days"
    );
  });
  ...
  it("User 6 should be able to claim amount after 35 Days", async () => {
    ...
    await tokenInstance._claimVestTokens(accounts[6], 9, expectedClaim_user6, {
      from: accounts[6],
    });
    ...
    assert.equal(
      balanceBefore_user6.toString(),
      balanceBefore.toString(),
      "Balance Before is Wrong"
    );
    assert.equal(
      balanceAfter_user6.toString(),
      balanceAfter.toString(),
      "Balance after is not right"
    );
    assert.equal(
      totalClaimed.toString(),
      totalClaimed_user6.toString(),
      "Total Claimed is wrong"
    );
  ...
  it("User 4 should not be able to claim amount after 65 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 65 days"
    );
  });
  ...
  it("User 4 should not be able to claim amount after 95 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 95 days"
    );
  });
  ...
  it("User 4 should not be able to claim amount after 125 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 125 days"
    );
  });
  ...
  it("User 4 should not be able to claim amount after 155 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 155 days"
    );
  });
  ...
  it("User 4 should not be able to claim amount after 185 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 185 days"
    );
  });
  ...
  it("User 4 should not be able to claim amount after 215 Days", async () => {
    ...
    assert.equal(
      actualClaims.toString(),
      user5_claims.toString(),
      "Claims do not match for 215 days"
    );
  });
});
